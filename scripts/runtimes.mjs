...
// shebang is node (and engines.node is >=22.12, puppeteer-core's own floor), and
// Deno runs the same compiled ESM via `deno run -A`. Some checks therefore have
// to actually execute them. Those skip with a notice when the runtime is absent,
// and gate normally where it exists, which is every CI runner.
//
...
// a hard failure: "setup-node didn't run" must not look the same as "passed".
