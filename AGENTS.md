...
- **The `release` branch is the plugin channel.** The marketplace repo pins
  `ref: release`, and the release workflow fast-forwards it after a successful
  publish. Don't push to it by hand — that would ship plugin content for a
  version that isn't on npm.
- **The plugin channel and the npm channel ship different content, on
  purpose.** The plugin cache is a git clone of the `release` branch. `dist/` is
  gitignored, so that clone has source (`.ts`), `skills/` and `.claude-plugin/`
  — never a built binary. The npm tarball is the mirror image: `files:` ships
